...
Next, execute `run_docker.sh launch_jup`. Run `docker ps` to confirm that the container has launched with the image name `gurudesh/copycat:copycat-cpu`. You should now be able to access the container via a CLI by running `run_docker.sh run_jup`.
